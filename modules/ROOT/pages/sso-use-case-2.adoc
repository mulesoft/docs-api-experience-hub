...
. From the browser, copy the 32 digit ID from the URL.
+
image::aeh-sso-32-digit-id.png[32 digit located in the browser URL] 
. Go back to Okta > Directory > Profile Editor* and click *Mappings*
. Scroll to end of list and paste the ID into the new anypoint_idp_id attribute. The ID must in quotes.
+ image::aeh-sso-mappings.png[Example of mapped anypoint_idp_id attribute]
. Click *Save Mappings*.

== Step 2: Add Salesforce Identity Providers 
//reused content from partial (ran out of time will do post GA)
Add and enable the Salesforce identity provider from the API Experience Hub UI. When the identity provider is enabled, users can log in to the portal using this identity provider.
