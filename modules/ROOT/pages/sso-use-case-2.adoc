= Use Case 2: Configuring SSO to Use the Identity Provider That is Already Configured for Anypoint Platform
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

Use case two includes instructions for configuring SSO for the portal using an identity provider already configured for Anypoint Platform. This use case also provides instructions for reconciling the xref:enabling-sso-for-your-portal.adoc#username-strategy[username assignment strategy] when the usernames for the portal and Anypoint Platform do not match. 
//The usernames in both organizations match in each identity provider. For example, the Salesforce and Anypoint Platform username is anyuser@salesforce.com. No extra steps to reconcile the usernames are necessary.
