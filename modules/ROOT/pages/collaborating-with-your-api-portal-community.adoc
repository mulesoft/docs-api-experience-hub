= Collaborating With Your API Portal Community
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

You can enable collaboration with members of a portal community using Salesforce Chatter Feed components to:

* Sharing posts and updates
* Participating in polling
* Asking questions 
* Receiving notifications for relevant activities in personalized feeds

The Chatter Feed options that display depend on how your administrator sets up feeds for the API portal.

To see feedback and discussions about a service:

. Select a services from the *Home* or *Services* page of the portal. 
. In the *API details* page, select the *Feedback and discussions* tab.
+
*Feedback and discussions* is the default name of the tab, however, administrators can change this name.

For more information about using Chatter Feed tracking, see the following Salesforce documentation:

* https://help.salesforce.com/s/articleView?id=experience.collab_overview.htm&type=5[Chatter Overview]
* https://help.salesforce.com/s/articleView?id=experience.collab_home_overview.htm&type=5[Chatter Tab Overview]
* https://help.salesforce.com/s/articleView?id=experience.collab_feed_posting.htm&type=5[Posting Overview]
* https://help.salesforce.com/s/articleView?id=sf.collab_feeds_parent.htm&type=5[Work with Feeds]



