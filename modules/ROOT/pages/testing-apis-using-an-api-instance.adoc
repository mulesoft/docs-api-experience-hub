...
The API Console enables you to test an API using an API instance as a server. API instances let you to interact with a server that sends requests with responses of actual data. You can also add an authorization header and search for assets your organization using that instance. If an instance has authorization headers, the request fails when the authorization headers are not entered.
