...
Consider the use case when you want to configure an identity provider as a login option for your portal and an identity provider is already configured in your Salesforce and Anypoint organizations. The usernames used in those organizations do not match for example, the Salesforce username is *anyuser@salesforce.com* and the Anypoint username is *anyuser*. To have a single user identity, you must reconcile the username format by adding attributes for *anypoint_idp_id* and *anypoint_username*.
