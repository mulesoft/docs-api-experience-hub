...
Adding and configuring custom user attributes for SAML or claims for OpenID Connect ensures that the identity provider and the user information are mapped correctly. The AEHPortalRegistrationHandler requires specific user information from the identity provider to create or update a user. The following required information lists the fields it's extracted from and the supported format:
