...
An identity provider for Anypoint and Salesforce can be configured using OpenID Connect or SAML (Single Sign-on Settings). 
...
* Reviewed Access Management's xref:access-management::external-identity.adoc[Identity Provider] documentation to understand identity management for Anypoint Platform.

[[username-strategy]]
=== Username Assignment Strategy
If you want to set up an SSO login option for your portal and your Anypoint and Salesforce organizations have identity providers already configured, review the username assignment strategy for your identity providers since they may be different. To have a single identity between Anypoint and Salesforce, the format for the username assignment must be the same. For Anypoint, you can use any character string however, Salesforce has a specific username strategy that uses an email format. If the username strategy does not match, custom attributes within the user profile must be added to the profile editor of the identity provider. Then, this information must be mapped with the Anypoint identity provider that you are using for SSO.
...
API Experience Hub supports the following use cases when configuring an identity provider as a login option for the portal: 

* xref:sso-use-case-1.adoc[Use Case 1: Configuring SSO When No Identity Provider is Already Configured] 
* Use Case 2: Configuring SSO When An Identity Provider is Already Configured for Anypoint and Salesforce and Usernames Match
* Use Case 3: Configuring SSO When An Identity Provider is Already Configured for Anypoint and Salesforce but Usernames Do Not Match

Instructions for configuring each use case varies, especially when adding or reconciling username assignments. 
