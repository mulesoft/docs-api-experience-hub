...
+
Configuring the user claims in the identity provider ensures that the user information is mapped correctly to Salesforce  and Anypoint users.  
+
The AEHPortalRegistrationHandler requires specific user information from the identity provider to create or update a user. The following tables list the required information and which claims the information is extracted from. If there is more than one claim for the same value, the registration handler looks for the first expected claim in the order it appears in the following tables:
+
...
|===
+ 
The following information is optional and enables you to configure specific use cases like xref:sso-use-case-2.adoc[Configuring SSO When the Identity Provider Is Already Configured for Anypoint Platform]:
+
