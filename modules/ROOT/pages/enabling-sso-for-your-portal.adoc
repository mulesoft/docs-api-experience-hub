...
* xref:configuring-sso.adoc#step-1-enable-sso-for-your-portal[Step 1: Enable SSO for Your Portal]
+
In this step, you create and configure an application in the identity provider. 
+
You can configure an identity provider for Anypoint Platform and Salesforce can be configured using OpenID Connect or SAML (Single Sign-on Settings). Microsoft Azure OpenID Connect is currently not supported.

* xref:configuring-sso#step-2-add-salesforce-identity-providers[Step 2: Add Salesforce Identity Providers]
+
...
image::aeh-sso-flow2.png["A diagram showing steps and applications required to configure SSO"] 
...
|===
+ 
To configure specific use cases, such as xref:sso-use-case-2.adoc[], use the following information:
+
...
|Anypoint Username|Explicitly specify the Anypoint Platform user when the user's identity already exists in Anypoint Platform, but the username is different from the Salesforce user. |anypoint_username
|===
+
If the required information isn't in the supported format, create a new custom attribute or claim with the supported format. For information, see the documentation for the identity provider. 
...
* xref:gathering-set-up-information.adoc[]
