...
Adding and configuring custom user attributes ensures that the identity provider and the user information are mapped correctly. The AEHPortalRegistrationHandler requires specific user information from the identity provider to create or update a user. The following information lists the required information, the fields it's extracted from, and the supported format:
