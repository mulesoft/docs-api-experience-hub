...
From the Service Details page, member users request access to an instance to which to register a client application. They do this by selecting an instance, selecting or creating a client application, and optionally selecting a service-level agreement (SLA). When the owner of the instance approves the request, a contract is created between the client application and the instance, and the client application is registered.

Instances that the client ID enforcement policy protects require client applications to provide a client ID and an optional client secret. Once the request for access is approved, the client application is registered and the client ID and client secret credentials are automatically created. Member users can request access to an instance of a service that is published in the portal. These users can either create a new application or use an application that they already created to request access.
...
== Before You Begin
Before you can request access to a service, you must be a member user and be logged into the portal. See xref:registering-for-access-to-portal.adoc[]

== Use an Existing Application to Request Access to a Service
You can request access to a service by using an existing application:

. Click a service to open the *Service Details* page.
...
. If the service has terms and conditions, select *I accept the terms and conditions*.
. Click *Request access*.
+
A Request access message displays to inform you that the request was received and is pending. If the SLA tier has auto-approval, the message shows that the request access was approved, and the application displays in the My Applications page. If the SLA tier has auto-approval, the application is received and approved.
...
== Create a New Application to Request Access to a Service
If you do not have an application to use to request access, create a new application: 

. From the Catalog, click a service to open the *Service Details* page.
. Click *Request access*.
. From the *Service instance* list, select an instance. 
...
. From the *OAuth 2.0 Redirect URIs* field, enter a URI address.
...
. Click *Request access*.
+
If auto-approval is enabled for the SLA tier, a message displays that the application is approved. If auto-approval isn't enabled, a message displays that the request is pending. When the application is approved, an email is sent.
. Click *Okay* to close the message.
