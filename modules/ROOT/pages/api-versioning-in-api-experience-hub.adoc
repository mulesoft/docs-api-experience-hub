...
+
For example, if the API version is v3, it becomes v4. If the Asset version is 3.0.1, it becomes 4.0.0. Name and Main file don't change.
...
. In API Experience Hub, go to *Catalog management*.
+
The API is still listed in *Services in the portal*. The new version has visibility *None*.
...
+
If the visibility is Everyone or Members only, then consumer developers can now see the new API version in the Carousel on the portal.
