...
Use Salesforce sandbox organizations to test your portal in a sandbox environment under the same conditions as your production environment. With API Experience Hub, you can manage separate deployment environments for your API portal. You can connect to multiple Salesforce sandbox organizations and switch between them. Switching doesn't impact the established connections with the Salesforce organization and Anypoint Platform. For example, if you switch from production to a sandbox, the connection with the production environment remains operational. The portal functions as it did before.

Manage Salesforce organizations from the *Manage your portal* page by selecting sandbox environments that are available in the current Salesforce connection.

== Before You Begin

. Create a public group:
.. Go to Salesforce > *Setup* > *Users* > *Public Groups*.
.. Click *New* and enter a label and group name.
.. From the *Available Members* list, select the members who you want to assign to the group. To move the selected members to the *Selected Members* list, click *Add*.
.. Save your changes. 
...
. In the *Manage your portal* page, click the connection control. 
+
image::aeh-org-switcher.png["The connection control to switch between environments",15%]
. Select an option:
+
...
* To switch to another sandbox for the connected production organization, click *Switch to this org* from the *Connected Salesforce Orgs* dialog.
+
...
To delete a sandbox organization, see https://help.salesforce.com/s/articleView?id=000383260&type=1[Delete a sandbox] in the Salesforce documentation.
...
.. Create a manifest `package.xml` file in `./manifest`, using this example with your information:
+
[%collapsible]
====
[codeblock]
...
[codeblock]
----
sf auth web login --instance-url=https://<SANDBOX-INSTANCE_URL> --alias sandboxOrg
----
.. Edit the `manifest/package.xml` file before retrieving the metadata from the sandbox organization. 
+
...
+
.. If the portal contains branding images, add them in the `ContentAsset` section of the `manifest/package.xml`. For example:
+
[codeblock]
----
...
...
or modify the `manifest/package.xml` and repeat the process if some metadata is missing or unnecessary.
...
.. Install the package in production if it's not installed already.
+
...
[codeblock]
----
sf project deploy start --manifest=manifest/package.xml --target-org productionOrg
----
+
...
.. From the *Catalog management* page, add assets to the portal.
...
* https://developer.salesforce.com/docs/atlas.en-us.communities_dev.meta/communities_dev/networks_migrate_overview.htm[Deploy an Experience Cloud Site from Sandbox to Production]
