...
When APIs are added to the portal, all administrators are automatically assigned the Exchange Viewer permission for the APIs added in the portal. 

To grant the permissions to an administrator:

//. Click the MuleSoft icon (image:aeh-mulesoft-logo.png[2%,2%]), and then select *Access Management*.
. Go to *Access Management*.
. Click *Users*, and then select the username of the user that you want to add the API Experience Hub Administrator permission. 
. Select *Permissions* and click *Add permissions*.
. In the *Select permissions to add for this user* search box, enter *API Experience Hub Admin* and select the checkbox.
. Enter *Exchange viewer* in the search box and select the checkbox, and then click *Next*.
. Select the *MuleSoft* checkbox, and then click *Next*.
...
.. Click *Edit*.
.. In the *Systems* section, enable the following _required_ permissions:
+
...
== Assign API Experience Hub Users Approver Permission
Assign the API Experience Hub Users Approver permission to enable a user to approve or reject pending registration requests and enable or disable member users only.  

This permission grants access to the following *User Management* pages in *Manage your API portal*:
...
* Settings
+
View information about the Salesforce account and portal.

To assign this permission to a user:

. Go to *Access Management*.
. Click *Users* and select a username. 
. Select *Permissions* and click *Add permissions*.
. In the *Select permissions to add for this user* search box, enter *API Experience Hub Users Approver* and select the checkbox.
. Click *Next*.
. Select the *Mulesoft* checkbox and click *Next*.
. Click *Add Permissions*.
