...
The following sections summarize the available options. For detailed information, refer to https://developer.salesforce.com/docs/atlas.en-us.communities_dev.meta/communities_dev/communities_dev_customize_css.htm[Override Template Elements with Custom CSS^] in the Experience Cloud Developer Guide.

== Edit the CSS

. In the *Builder*, click the paintbrush icon to open the *Theme* menu.
. Click the drop-down arrow of the menu and click *Edit CSS*.
. Edit the CSS file in the *Edit CSS* dialog box.
. When you're finished with your changes, click *Save*.
...
=== API Catalog
This component shows all the assets in the site. You can search assets and filter by asset type.
...
`.credentials-tooltip-title`	
|Client ID and client secret label.
|`.applications-listing-table-row-credentials `

`.credentials-tooltip-code`	
...
It shows pages from an asset. These pages are requested from Exchange.
...
|Spinner container that's executed while the documentation request gets information since the last time listed in the asset detail header.
|`.asset-documentation-viewer-spinner-container`
 
`.asset-documentation-viewer-spinner`	
|Spinner that's executed while the documentation request gets information since the last time listed in the asset detail header.
...
=== Asset Detail Header
(Public component) This component is on the API detail page. It shows basic information about an asset (logo, name, asset type, information contact, combo box version, status, instances, clients, spec download, and request access button)
...
|In some errors, it shows the request ID.
...
|Container for request access modal footer.
