...
Anypoint Platform comes with predefined roles and permissions that can be set in Access Management to manage users with different access levels. API Experience Hub has permissions for administrators, consumers, and Exchange viewers.
...
Once your Anypoint organization is linked to your Salesforce organization, two teams are created:
...
+
Any user assigned as the API Experience Hub Portal Administrator is automatically added to this team in Access Management.

Administrators can only add APIs that they have the Exchange Viewer permission for them. When APIs are added to the portal, all administrators are automatically assigned the Exchange Viewer permission. 
...
* Manage users
* Use Experience Builder to customize portal|
API Experience Hub Portal Administrator| Currently, manages only one API portal in the organization. In a later release, it may be possible to create more that one portal per organization, for example, one internal and one external portal. 
a|
