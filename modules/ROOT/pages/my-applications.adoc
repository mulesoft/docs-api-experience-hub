...
The My Applications page lists applications that hold a contract with an API. Each client application has a client ID, client secret, description, URL, redirect URIs,  and grant types.
...
If you don’t have an application to link to the API, you can create the application in the Request access dialog. You can then click Request access to connect your application and the API. Some APIs require selecting a service-level agreement (SLA), and some APIs require accepting terms and conditions.
...
API consumers use the My Applications page to view the contracts created between an API instance and that consumer’s client applications.

== Search for Applications
If you have several applications listed in the My Applications page, you can search for a specific one by using the Search box. Enter the application name or the description of the application and press enter or return. The number of applications that match your search criteria display in the list of search results.
...
* To copy the client ID or the client secret to the clipboard, click the Copy to clipboard icon.
...
* API Contracts
+
Lists the application’s API name, environment and instances, SLA tier, and request access state.  The environment can be either a sandbox or development environment. Define SLA tiers in API Manager. Request access states can be pending, approved, or failed.

//screen capture of an example Application Details page

From Application Details page, you can:
...
. From *Client Credentials*, click *Reset client secret*. 
. Click *Confirm*.
...
. From *My Applications*, click the application name in the list.  
. Click *Edit*.
. From *Application Details*, edit the corresponding fields.
. Click *Save*.
...
. Click the name of the application in the list from the My Applications page.
. Click + Add.
. From *Add SLA Tier*, select from the list of tiers.
. Click *Save*.
+
...
. From the *SLA Tier* column of the API Contracts section that you want to change, click the SLA tier option. 
. From the *Edit SLA Tier* dialog, select from the list of SLA tiers.
. Click *Save*. 
. Click *Okay* to close the message.

=== Delete a Client Application
When you do not need the contract for an API, you can delete it. Once the application is deleted, there is no longer a contract with the API and you cannot access the it. To regain access, you must request access to the API again.
