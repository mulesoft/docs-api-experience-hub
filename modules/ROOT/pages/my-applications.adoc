...
* Last Updated
+
Displays the date when the application credentials (client ID and client secret) were last reset.
* API Contracts
+
...
From the Application Details page, you can reset the client secret for an application. Applications using the current client secret can't access any of the APIs with which they were registered.
...
NOTE: After resetting the client secret, the *Last Updated* date on the Application Details page reflects the date of the reset.
...
When you no longer need the contract for an API, you can delete it. Once the application is deleted, there is no longer a contract with the API and you can't access it. To regain access, you must request access to the API again.
