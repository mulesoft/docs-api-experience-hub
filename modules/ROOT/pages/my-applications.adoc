...
The *My Applications* page enables you to review the lists of applications that hold a contract with a service. Each client application has a client ID, client secret, description, URL, redirect URIs, grant types, and last updated date.

Services that are protected by a client ID enforcement policy require client applications to provide a client ID and optional client secret. The client ID and client secret credentials are automatically created when the client application is registered. For more information about registering an application, see xref:requesting-access-to-an-api-instance.adoc[].
...
* Contracts
+
From the Contracts section, you can see the application’s service name, environment and instances, instance URLs, SLA tier, and request access state. The environment can be either a sandbox or development environment. Request access states can be `pending`, `approved`, or `failed`.
...
=== Reset Client Secret
You can reset the client secret for an application on a regular cadence to ensure security when users leave the organization or to ensure that the secret isn't accidentally exposed to unauthorized users.  

From the *Application Details* page, you can reset the client secret for an application. Applications using the current client secret can't access any of the services with which they were registered.
...
To view the instance URL for a service by clicking *View URL* and copy it by clicking the *Copy from clipboard* icon.
...
. From the *My Applications* section, click the application name in the list. 
. From the *SLA Tier* column of the *Contracts* section, click + Add.
...
=== Change the SLA Tier
The SLA tier can limit the number of requests an application can make to the service. You can increase or decrease the number of allowed requests. You can change the SLA tier for an application from the Application Details page. The available SLA tiers are set by an admin in API Manager. The owner of the API instance must approve the request to change the tier level.

To edit the SLA tier for a client application:

. On the *My Applications* page, click the name of the application in the list.
. From the *SLA Tier* column of the *Contracts* section that you want to change, click the SLA tier edit option. 
. From the *Edit SLA Tier* dialog, select from the list of SLA tiers.
. Click *Save*. 
. Click *Okay* to close the message.

=== Delete a Client Application
When you no longer need the contract for a service, you can delete it. Once the application is deleted, there is no longer a contract with the service and you can't access it. To regain access, you must request access to the service again.
