...
An API Experience Hub Admin team and an API Experience Hub Portal Admin team is created during the connecting and linking process. Once your Salesforce organization is successfully connected to your Anypoint organization, the AEH Admin team is created. After the Salesforce linking process is complete, any user, who is assigned the AEH Admin, is automatically added to the AEH Admin team. Once your styling changes for the portal are saved, the AEH Admin team is created. 
Although the API Experience Hub Portal Administrator team is visible in Access Management, this permission is not supported in this release.
