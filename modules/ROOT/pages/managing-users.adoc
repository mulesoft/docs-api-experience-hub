...
Manage portal members using the *Portal members* page. When a guest user's registration or an existing Salesforce user is approved, the user's name is added to the list of members. You can view a searchable list of current members and manage their accounts by enabling or disabling them and adding them to user groups.
...
Assign users to user groups to manage which services they can see in your portal. You can assign a user to multiple user groups or change user groups.

. From the *Portal members* page, locate the member's name in the list and click *View and assign user groups*.
. Select one or more user groups and click *Apply*.

== Manage Pending Registration Requests
Admins must manually approve or reject requests for users who want to access the portal using the *Pending registration requests* page. You can view the name, email, registration date, and registration status of the users who register for access. By default, users are assigned to the Members profile. You can add them to the available user groups that can see specific services in the portal. The maximum pending user registration requests is 500.
...
After a guest user registers or an existing Salesforce user requests access, approve or reject the requests and assign the user to user groups. All users who self-register are assigned to the Members profile as well as existing Salesforce users who request access. You can assign member users to multiple user groups.
...
== Manage User Groups
Create and manage a group of users who can see specific services in your portal using the *User groups* page. The maximum number of user groups is 20.

Before deleting a user group, remove all users, services, and the visibility for all instances assigned to the user group. 

* To create groups of users who can see specific services in the portal:
...
. Enter the group name and select a Salesforce identity provider name.
. Click *Save*.

=== View Services That User Groups Can See
To view the services that are visible by a user group for portal members and also the APIs assigned only to a user group. The *Services visible to the user group* tab shows all of the APIs that members of the portal can see.  

. From the *User groups* page, select a user group.
. Select the *Services visible to the user group* tab.
. To see services assigned to the group only, turn on *Services assigned to this group*.

To view or change a service's visibility, see xref:managing-apis.adoc#make-api-versions-visible[Make API Versions Visible].
...
Disable or enable self-registration for external users and set up single sign-on for your portal using the *Login settings* page. Self-registration enables guest users to register themselves as members of your portal. From the *Pending requests* page, you can see a list of pending requests from guest users who want to access your portal. 
