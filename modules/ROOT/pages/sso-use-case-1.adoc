...
Configuring the custom user attributes for the identify provider ensures that the identity provider and the user information are mapped correctly. For example, the AEHPortalRegistrationHandler requires specific user information from the identity provider to create or update a user. The following required information lists the fields it's extracted from and the supported format:
...
If the required information is not in the supported format, create a new attribute with the supported format. For information, see the documentation for the identity provider. 
