...
* Cards to display
+
The number of API cards to display in the carousel. A card represent an API in the portal. The carousel can display a minimum of three cards and a maximum of fifteen cards. If the number of cards to display is five and the portal has only three APIs, then the carousel can display three cards only.

=== Edit the Titles in the API Carousel
You can modify the title and subtitle that appear for the API Carousel. You can also show the number of cards in the subtitle by entering the `{numberOfAssets}` variable in the *Subtitle* field. For example 

image::aeh-builder-carousel-subtitle.png[Image of Asset Carousel Subtitle option]
...
. If you want to include the number of assets in the subtitle, enter `{numberOfAssets}` to the *Subtitle* field. For example, Last APIs added 
