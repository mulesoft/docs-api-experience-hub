...
+
To configure the redirects, use your 15-digit organization ID and 18-digit org ID and add a URL for each organization ID.
...
. From Okta, create a SAML application and enter a name for your SAML application in *General Settings*.
...
|*Audience URI (SP Entity ID)* |Enter https with the Salesforce domain for example, `\https://{salesforcedomain}.com`.
...
. Enable multi-valued SAML attributes when sending multiple groups in the groups claim. 
+
Sending groups as a comma-separated list isn't required.
+
.. Navigate to *Salesforce* > *Setup*.
.. From *Setup*, in the Quick Find box, enter `Custom Metadata Types`, and then select *Custom Metadata Types*.
.. Click *Manage Records* for *API Experience Hub Configuration*.
.. Click *Edit* for the *Default* record, select the *Enable Multi-Valued SAML Attributes* checkbox, and then click *Save*.
...
.. In *Browse Microsoft Entra Gallery*, select the *Salesforce* application.
...
[NOTE]
====
API Experience Hub uses the value of the `\http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name` claim as the username, which must be email-shaped for Salesforce to accept it. The Microsoft Entra ID `user.userprincipalname` attribute is email-shaped by default. If this claim is not present, API Experience Hub uses the value of the email claim instead.
====
.. Save your changes.
. Enable multi-valued SAML attributes when sending multiple groups in the groups claim. 
+
