...
If this single sign-on is already configured in Anypoint Platform, you must map the application to the identity provider for Anypoint Platform you must map the identity provider ID with the anypoint_idp_id by adding and configuring a new custom attribute called anypoint_idp_id.   
