...
NOTE: Skip this step if you're configuring a new identity provider for Anypoint Platform. Go to xref:configuring-sso.adoc#add-identity-provider[Step 2: Add Salesforce Identity Providers].
...
. In the *Attribute Statements (optional)* section, configure the SAML integration by adding these custom attributes. For supported formats, see the supported format for user attributes listed in xref:xref:before-you-begin-configuring-sso.adoc[]:
+
...
Create a new application for the API Experience Hub portal in the identity provider using Microsoft Azure. Microsoft Azure OpenID Connect isn't supported.
...
*Map the Application to the Identity Provider for Anypoint Platform*
//Note to editor: Will revisit whether to make lines 319, 321, and 323 partials if the content is still relevant once we release a single identity in SF. 
NOTE: Skip this step if you are configuring a new identity provider for Anypoint Platform. Go to xref:configuring-sso.adoc#step-2-add-identity-providers[Step 2: Add Salesforce Identity Providers].

If this single sign-on is configured in Anypoint Platform, map the application to the identity provider for Anypoint Platform using a new custom attribute, `anypoint_idp_id`.   
...
. Map `anypoint_idp_id` to the Anypoint Platform identity provider ID for Microsoft Azure SAML:
