...
There can be a a few issues that can cause this error to occur:

* The external identity provider group mapping is missing from Access Management.
* The *Auth provider Default Scopes* field is not configure properly.
...
When trying to log in to the portal with an identity provider, you see a Problem Logging In error that the third-party identifier cannot be be found. A URL that is not configured properly can cause issues logging in.

To troubleshoot this issue, ensure that the URL is not properly configured. For Okta, the `/userinfo` part of the URL path should be lowercase.
