...
There is no option to log in to the portal through the identity provider after the identity provider is configured, mapped, and enabled in the API Experience Hub UI. Currently, the information entered when setting up SSO cannot be verified from API Experience Hub. Errors can occur and prevent end users from logging in to the portal using this method.
...
* The correct user group was not added in Access Management.
...
. To make changes, click *Edit* and then click *Save*.
