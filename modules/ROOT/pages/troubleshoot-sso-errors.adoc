...
=== Auth Provider Default Scopes Field Is Not Configured Properly
...
== Cannot Log In
When logging in to the portal with an identity provider,
the portal shows the following errors:

* Problem Logging In Error
* Single Sign-On Error

=== Problem Logging In Error
You see a *Problem Logging In* error that the third-party identifier cannot be be found. An incorrect portal URL can cause issues logging in.
...
== Single Sign-On Error
Misconfiguring single sign-on or the Salesforce Auth. Provider can cause this error. 

A few issues can cause this error to occur:

* The Entity ID isn't the same in both the Salesforce Auth. Provider and the identity provider.
* Making changes to Single Sign-On Settings without uploading the SSO certificate. 

=== Entity ID Isn't the Same in the Salesforce Auth. Provider and the Identity Provider
 
To troubleshoot this issue for the SAML protocol configurations, ensure the *Entity ID* is the same in both the Salesforce Auth. Provider's *Single Sign-On Settings* page and the identity provider. 

Click *SAML Assertion Validation* in the *Single Sign-On Settings* page to check for errors in the assertion.

=== Making Changes to Single Sign-On Settings Without Uploading the SSO Certificate

SSO sing-on errors can occur if you make changes to the *Single Sign-On Settings* page without uploading the SSO certificate. 

To troubleshoot this issue, ensure you always upload the certificate when making changes to the page.
