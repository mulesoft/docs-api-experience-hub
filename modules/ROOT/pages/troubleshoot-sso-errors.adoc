...
. From the *Manage your portal* page, click *Preview and publish your portal*.
. Click *Publish*. 

== Carousel Not Loading

After logging in to the portal successfully, you can't see services in the Carousel.
...
* Duplicate user accounts or single sign-on errors. 
+
Users who already have an active profile in Salesforce are unable to log in via Just-In-Time (JIT) provisioned Single Sign-On (SSO).
New users (those who don't exist in the Salesforce system yet) can log in successfully, but existing users are blocked.
+
Errors occurs because the API Experience Hub configuration is missing the specific user profiles of the users trying to log in. When the SSO attempt is made, the system doesn't see the user's profile as authorized for that specific API Experience Hub instance, causing the authentication process to fail for those established users.
+
To troubleshoot this issue, follow the instructions in the Salesforce article, https://help.salesforce.com/s/articleView?id=005316413&type=1[API Experience Hub - Salesforce Users Unable to Log In to API Experience Hub Using JIT-Provisioned SSO] to fix the issue.
...
+
. Make sure the *Entity ID* is the same in both the Salesforce Auth. Provider's *Single Sign-On Settings* page and the identity provider. 
+
...
+
To troubleshoot this issue, make sure you have the expected claims configured in the application. For more information, see xref:gathering-set-up-information.adoc#required-claims-and-attributes-for-sso[Required Claims and Attributes].
...
. Delete the Anypoint Platform token from the Named Credential.
+
